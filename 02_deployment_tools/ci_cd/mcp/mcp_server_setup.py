...
"""
MCPサーバ拡張設定スクリプト
リモート操作（ファイル作成、コマンド実行等）を可能にするMCPサーバの設定
"""
...
# メイン処理
if __name__ == "__main__":
    print("MCP Server Extended 設定ファイル生成")
    print("=" * 50)
    ...
    print(f"[OK] 設定ファイル作成: {config_path}")
    
    # サーバスクリプト作成
    server_script = create_mcp_server_script()
    server_path = "mcp_server_extended.py"
    with open(server_path, 'w') as f:
        f.write(server_script)
    print(f"[OK] サーバスクリプト作成: {server_path}")
    
    # インストールスクリプト作成
    install_script = create_installation_script()
    install_script = install_script.replace("<<<CONFIG_CONTENT>>>", json.dumps(config, indent=2))
    install_script = install_script.replace("<<<SERVER_CONTENT>>>", server_script)
    
    install_path = "install_mcp_extended.sh"
    with open(install_path, 'w') as f:
        f.write(install_script)
    print(f"[OK] インストールスクリプト作成: {install_path}")
    
    print("\n" + "=" * 50)
    print("次のステップ:")
    print("1. install_mcp_extended.sh をMCPサーバにコピー")
    print("2. bash install_mcp_extended.sh を実行")
    print("3. sudo systemctl start mcp-server でサーバー起動")